      ...
      <header className="fixed top-0 left-0 right-0 z-50 flex justify-center px-4 py-4 sm:py-6 pointer-events-none">
        <motion.nav
          ...
          className={cn(
            ...
            scrolled
              ? 'bg-black/60 backdrop-blur-2xl border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_15px_rgba(0,214,255,0.08)]'
              : 'bg-black/40 backdrop-blur-xl border border-white/[0.06] shadow-[0_4px_24px_rgba(0,0,0,0.3)]'
          )}
        >
          {/* Brand Monogram & Name using TechLogo */}
          <TechLogo size="sm" href="/" />
                ...
                >
                  {isActive && (
                    <motion.div
                      ...
                      className="absolute inset-0 rounded-full bg-white/[0.08] border border-cyan-electric/30 shadow-[0_0_12px_rgba(0,214,255,0.15)]"
          ...
          <div className="flex items-center gap-2">
            <Link
              href="/contact"
              className="group relative inline-flex items-center gap-1.5 px-3.5 sm:px-4 py-1.5 rounded-full text-xs font-medium tracking-wide bg-gradient-to-r from-cyan-electric/20 to-blue-corporate/20 border border-cyan-electric/40 text-cyan-electric hover:border-cyan-electric hover:shadow-glow-cyan transition-all duration-300 active:scale-95"
            >
              <span>Inquire</span>
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-2">
              <div className="px-3 py-2 text-[10px] tracking-widest text-cyan-electric/70 uppercase border-b border-white/5 font-mono">
                Mandili Hardware Portfolios
              </div>
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive
                        ? 'bg-white/10 text-cyan-electric border border-cyan-electric/30'
                        : 'text-neutral-300 hover:text-white hover:bg-white/5'
                    )}
                  >
                    <div className="flex items-center gap-3">
                      <Icon className="w-4 h-4 text-cyan-electric" />
                      <span>{item.name}</span>
                    </div>
                    {isActive && (
                      <span className="w-1.5 h-1.5 rounded-full bg-cyan-electric shadow-[0_0_8px_#00D6FF]" />
                    )}
                  </Link>
                );
              })}
              <div className="pt-2 mt-2 border-t border-white/5">
                <Link
                  href="/contact"
                  onClick={() => setMobileMenuOpen(false)}
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-cyan-electric text-obsidian-950 font-semibold text-xs uppercase tracking-wider hover:bg-white transition-colors"
                >
